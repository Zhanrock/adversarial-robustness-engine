...
class TestConfig(unittest.TestCase):
    ...
    def test_basic_load(self):
        p = self._yaml(YAML); cfg = load_config(p); os.unlink(p)
    ...
    def test_missing_file_raises(self):
        with self.assertRaises(FileNotFoundError): load_config("/no/such.yaml")
    def test_inheritance(self):
        with tempfile.TemporaryDirectory() as d:
            base  = os.path.join(d,"base.yaml")
            child = os.path.join(d,"child.yaml")
            with open(base,"w")  as f: f.write("base_key: 42\noverride_key: original\n")
            with open(child,"w") as f: f.write("_base_: base.yaml\noverride_key: overridden\nnew_key: hi\n")
