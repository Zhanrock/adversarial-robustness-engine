...
class TestASR(unittest.TestCase):
    def test_perfect_attack(self):
        yc = np.array([0,1,2,3]); ya = np.array([1,2,3,0]); yt = np.array([0,1,2,3])
    ...
    def test_already_wrong_not_counted(self):
        yc = np.array([0,9,2,9]); ya = np.array([1,9,3,9]); yt = np.array([0,1,2,3])
        self.assertEqual(attack_success_rate(yc, ya, yt), 1.0)
...
class TestComputeAllMetrics(unittest.TestCase):
    ...
    def test_all_keys_present(self):
        ...
        for k in ["clean_accuracy","adversarial_accuracy","attack_success_rate",
                  "robustness_gap","mean_l2_perturbation","certified_robustness"]:
    ...
    def test_gap_consistent(self):
        ...
        self.assertAlmostEqual(m["robustness_gap"],
                               m["clean_accuracy"]-m["adversarial_accuracy"], places=5)
