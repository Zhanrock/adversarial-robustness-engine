...
class TestGaussianDenoiser(unittest.TestCase):
    def test_contract(self):
        _contract(self, GaussianDenoiser(0.05), X)
...
class TestMedianDenoiser(unittest.TestCase):
    def test_contract(self):
        _contract(self, MedianDenoiser(3), X)
...
class TestFeatureSqueezing(unittest.TestCase):
    def test_contract(self):
        _contract(self, FeatureSqueezing(4), X)
...
class TestDefensePipeline(unittest.TestCase):
    def test_applies_in_order(self):
        d1 = GaussianDenoiser(0.05)
        d2 = FeatureSqueezing(6)
...
class TestAdversarialTrainer(unittest.TestCase):
    ...
    def test_training_runs(self):
        x, y = self._data()
        h = AdversarialTrainer(MODEL, attack="fgsm", epsilon=0.05, epochs=2, batch_size=16).fit(
            x, y
        )
    ...
    def test_metrics_in_range(self):
        ...
        h = AdversarialTrainer(
            MODEL, attack="pgd", epsilon=0.03, epochs=2, batch_size=16, pgd_steps=3
        ).fit(x, y)
        for e in h.epochs:
    ...
    def test_invalid_ratio_raises(self):
        with self.assertRaises(ValueError):
            AdversarialTrainer(MODEL, ratio=1.5)

    def test_invalid_attack_raises(self):
        with self.assertRaises(ValueError):
            AdversarialTrainer(MODEL, attack="cw")
