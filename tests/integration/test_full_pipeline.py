...
class TestBenchmarkSerialization(unittest.TestCase):
    def test_json_written(self):
        with tempfile.TemporaryDirectory() as d:
            b = RobustnessBenchmarker(MODEL, defenses=[GaussianDenoiser(0.05)],
                                      output_dir=d, model_name="test")
...
class TestAdversarialTrainingCycle(unittest.TestCase):
    def test_cycle(self):
        xt,yt = X[:48], Y[:48]; xv,yv = X[48:], Y[48:]
        h = AdversarialTrainer(MODEL, attack="fgsm", epsilon=0.03,
                               ratio=0.5, epochs=2, batch_size=16).fit(xt,yt,xv,yv)
...
class TestMultiAttackBenchmark(unittest.TestCase):
    def test_all_attacks_present(self):
        attacks = [FGSM(MODEL,0.05), PGD(MODEL,0.05,num_steps=5),
        ...
        self.assertIn("FGSM", names); self.assertIn("PGD", names); self.assertIn("PatchAttack", names)
    ...
    def test_report_gap_consistent(self):
        ...
        res = r.results[0]
        self.assertAlmostEqual(res.robustness_gap,
                               res.clean_accuracy - res.adversarial_accuracy, places=5)
