...
class TestFGSMContract(unittest.TestCase):
    def test_contract(self):
        _contract(self, FGSM(MODEL, EPS), X, Y)
...
class TestPGDContract(unittest.TestCase):
    def test_contract(self):
        _contract(self, PGD(MODEL, EPS, num_steps=5), X, Y)
    ...
    def test_invalid_alpha_raises(self):
        with self.assertRaises(ValueError):
            PGD(MODEL, EPS, alpha=0.0)

    def test_invalid_steps_raises(self):
        with self.assertRaises(ValueError):
            PGD(MODEL, EPS, num_steps=0)


class TestPatchAttackContract(unittest.TestCase):
    def test_contract(self):
        _contract(self, PatchAttack(MODEL, patch_size=4, num_steps=10), X, Y)
